...
'''
Generates 2D plots (m_phi vs m_tphi) of the final signal efficiencies, as well as the efficiencies 
before and after the lepton veto.
'''
...
def GetEfficiencies(year):
	...
	TChain.GetEntry()
        start = TChain.GetLeaf('PreLepVeto').GetValue(0)
        end = TChain.GetLeaf('PostLepVeto').GetValue(0)
    ...
    ax.set_title("Lepton Veto Signal Efficiency 20{}".format(year))
    ax.set_aspect('auto')
    plt.xlabel(r"$m_{\phi}$ [GeV]",fontsize='large')
    plt.ylabel(r"$m_{t\phi}$ [GeV]",fontsize='large')
    plt.savefig('plots/LeptonVetoSignalEfficiency_{}.png'.format(year),dpi=300)
