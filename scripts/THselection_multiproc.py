...
@profile
def make_region_templates(queue, selection, node, region, HbbVar, TopVar, category):
    print('------------------------------------------------------------------------------------------------')
    print(f'Making {region} region templates in a separate subprocess')
    print('------------------------------------------------------------------------------------------------')
    ...
    # create the ttbarCR for the final search.                                                 print('Defining Fail and Pass categories based on Phi candidate score in %s...'%region)
...
def selection(args):
    ...
    if (args.njobs == '1'):
        ...
        outFileName = 'rootfiles/THselection_HT{}_{}_{}{}_{}of{}.root'.format(args.HT, args.setname, args.year, '_'+args.variation if args.variation != 'None' else '',args.ijob,args.njobs)
    ...
    # Define a Queue() in which to store all of the template histos for the regions
    queue = multiprocessing.Queue()
    # Spawn a subprocess in which to produce the templates for each of the regions
    for region in ['SR','CR','ttbarCR']:
        process = multiprocessing.Process(target=make_region_templates, args=(queue,selection,kinOnly,region,HbbVar,TopVar,category))
        process.start()
        process.join() # make script wait until child process is done

    # Prepare a root file to save the templates
    out = ROOT.TFile.Open(outFileName, 'RECREATE')
    out.cd()

    # Iterate over the queue to obtain the templates
    queue.put(None) # tells the iterator where to stop
    for template in iter(queue.get, None):
        print(f'Writing template histos from HistGroup {template.name}')
        template.Do('Write') # Event loop should have already been triggered in the MakeTemplateHistos() 
...
if __name__ == '__main__':
    ...
    if not args.singleproc:
        print('---------------------------------------------------------------------')
        print('     Running using python multiprocess module                        ')
        print('---------------------------------------------------------------------')
